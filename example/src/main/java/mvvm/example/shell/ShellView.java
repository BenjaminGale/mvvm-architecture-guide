package mvvm.example.shell;

import javafx.scene.layout.BorderPane;
import javafx.scene.layout.Region;
import mvvm.example.core.view.ViewLocator;
import mvvm.example.shell.statusbar.StatusBarView;
import mvvm.example.shell.tabs.TabView;
import mvvm.example.shell.toolbar.ToolbarView;
import mvvm.example.shell.workspace.WorkspaceSidebarView;

public class ShellView extends BorderPane {

    public ShellView(ShellViewModel viewModel, ViewLocator<Region> viewLocator) {
        var content = new BorderPane();
        content.setTop(new ToolbarView(viewModel));
        content.setCenter(new TabView(viewModel, viewLocator));

        setLeft(new WorkspaceSidebarView(viewModel));
        setCenter(content);
        setBottom(new StatusBarView(viewModel, viewLocator));
    }
}
